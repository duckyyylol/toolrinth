import { ApplicationCommandOptionType, ApplicationIntegrationType, ButtonStyle, ComponentType, ContainerBuilder, inlineCode, InteractionContextType, makeURLSearchParams, MessageFlags, userMention } from "discord.js";
...
const MyProjectsCommand: Command = {
  enabled: true,
  name: "my_projects",
  description: "List your own projects on Modrinth",
  ...
  run: async interaction => {
    ...
    if (tokenUserRes.error) {
      await interaction.editReply({flags: [MessageFlags.IsComponentsV2], components: [RinthComponentBuilder.errorContainer(false, `${tokenUserRes.error.status} ${tokenUserRes.error.message}`).buildContainer()]})
      return;
    }

    const tokenUser = tokenUserRes.data;

    const userProjectsRes = await authorizedClient.Users().getUserProjects(tokenUser.id);
    ...
    for (let i = 0; i < userProjectsRes.data.length * 30; i++) {
      let pr = (i % 2 === 0) ? userProjectsRes.data : userProjectsRes.data.reverse();

      devProjects.push(...pr)
    }

    devProjects = devProjects.filter(p => p !== null);

    let userProjects = process.argv.includes("-dev") ? devProjects : userProjectsRes.data;

    if (projectTypeFilter) userProjects = userProjects.filter(p => p.project_type === projectTypeFilter);

    if (userProjects.length <= 0) {
      await interaction.editReply({flags: [MessageFlags.IsComponentsV2], components: [RinthComponentBuilder.errorContainer(false, `You have no projects${projectTypeFilter ? ` matching the filter type \`${projectTypeFilter}\`` : ""}`).buildContainer()]})
      return;
    }
...
export default MyProjectsCommand;
